...
def sample_tokens(logits, temperature=0.0, top_p=None, top_k=None, margin_confidence=False, neg_entropy=False, use_ori_logits=False):
...
class DreamGenerationMixin:
    ...
    def _sample(
        ...
        if attention_mask is not None and torch.any(attention_mask == 0.0):
            ...
            attention_mask = torch.ones_like(x, device=x.device, dtype=torch.float)
            tok_idx = attention_mask.long().cumsum(-1) - 1
            tok_idx.masked_fill_(attention_mask == 0, 1)
        ...
        # print('using mask free mask')
        for i in range(steps):
            # print('=' * 20, 'for mask free generation')
            ...
            if alg == 'origin':
                ...
                if alg == 'maskgit_plus' or alg == 'maskgit_plus_ar' or alg == 'ar':
                    ...
                    confidence, x0 = sample_tokens(mask_logits, temperature=temperature, top_p=top_p, top_k=top_k, use_ori_logits=True)
                elif alg == 'topk_margin':
                    confidence, x0 = sample_tokens(mask_logits, temperature=temperature, top_p=top_p, top_k=top_k, margin_confidence=True)
                elif alg == 'topk_margin_ori':
                    confidence, x0 = sample_tokens(mask_logits, temperature=temperature, top_p=top_p, top_k=top_k, margin_confidence=True, use_ori_logits=True)
                elif alg == 'entropy':
                    confidence, x0 = sample_tokens(mask_logits, temperature, top_p=top_p, top_k=top_k, neg_entropy=True)
                elif alg == 'entropy_ori':
                    confidence, x0 = sample_tokens(mask_logits, temperature, top_p=top_p, top_k=top_k, neg_entropy=True, use_ori_logits=True)
                else:
                ...
                if number_transfer_tokens > 0:
                    ...
                    if alg == 'maskgit_plus_ar' or alg == 'ar':
                        ...
                        _, transfer_index = torch.topk(full_confidence, number_transfer_tokens)  # NOTE: ignoring alg_temp
