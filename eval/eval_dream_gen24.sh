...
# tasks="minerva_math bbh"
# nshots="4 3"
# lengths="512 512"
# temperatures="0 0"

model=/data5/xyf/Dream/outputs/dream_transmla+math_linear_2000_finemath_4plus_filtered_linear_weight-eos-false_cpt_adapt_bs1024_step1600_cosine_lr1e-5/global_step_1600
mask_free=True
...
#     --model_args pretrained=${model},max_new_tokens=512,diffusion_steps=512,temperature=0.2,top_p=0.95,add_bos_token=true,escape_until=true,dtype=bfloat16,mask_free=${mask_free} \
#     --tasks humaneval \
#     --num_fewshot 0 \
#     --batch_size 2 \
...
for i in "${!TASKS_ARRAY[@]}"; do
    ...
    accelerate launch eval.py --model dream \
        --model_args pretrained=${model},max_new_tokens=${LENGTH_ARRAY[$i]},diffusion_steps=${LENGTH_ARRAY[$i]},add_bos_token=true,temperature=${TEMP_ARRAY[$i]},top_p=0.95,dtype=bfloat16,mask_free=${mask_free} \
        --tasks ${TASKS_ARRAY[$i]} \
