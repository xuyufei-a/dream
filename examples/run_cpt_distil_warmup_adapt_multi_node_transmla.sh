...
enable_cutoff=true
weight_eos=true
distil_loss_weight=1

dataset_name=transmla+math_1in2
base_model_path=../huggingface/Dream-v0-Base-7B
save_path=outputs/distil_warmup_dream_${distil_loss_weight}_${dataset_name}_${time_reweighting}_use_focal_${use_focal_loss}_noise_dist_${noise_dist}_enable_cutoff_${enable_cutoff}_bs${BATCH_SIZE}_step${STEP}_${lr_scheduler}_lr${LR}_weight_eos_${weight_eos}
...
torchrun \
    ...
    --nproc-per-node=8 \
    --node_rank=$NODE_RANK \
    -m src.trainer.fsdp_cpt_distil_warmup_trainer \
    ...
    diffusion.distil_loss_weight=${distil_loss_weight} \
    2>&1
